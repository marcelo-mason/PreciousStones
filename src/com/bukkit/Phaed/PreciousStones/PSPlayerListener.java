...
public class PSPlayerListener extends PlayerListener
{
	...
	for (Block source : sources.values())
	{
	    ...
	    if (psettings != null && !hasFastDamaged && psettings.fastDamage && !PreciousStones.Permissions.Security.permission(player, "preciousstones.bypass.damage"))
	    {
	...
	if (split[0].equalsIgnoreCase("/pstone") || split[0].equalsIgnoreCase("/ps"))
	{
		...
		else if (split[1].equals("removeall") && PreciousStones.Permissions.Security.permission(player, "preciousstones.whitelist.removeall"))
		{
		    String playerName = split[2];
